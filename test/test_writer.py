...
def test_writer_outputs_can_put_to_bucket(s3):
    ...
    s3.put_object(Body=test_csv, Bucket="test_bucket", Key="test_csv.csv")
    s3.put_object(Body=test_json, Bucket="test_bucket", Key="test_json.json")
    s3.put_object(Body=test_parq, Bucket="test_bucket", Key="test_parquet.parquet")
    bucket_contents = s3.list_objects_v2(Bucket='test_bucket')["Contents"]
    assert len(bucket_contents) == len(["test_csv.csv","test_json.json","test_parquet.parquet"])